...
using Sobol
using BasisMatrices, QuantEcon, Roots
using DataFrames
using MAT
using Parameters
using Printf
...
function Base.step(m, J, θ, u_lag, ξx, ξB, ξα, ξη)
    p, g = m.p, m.g
...
function initial_coefs(m, deg)
    ...
    coefs[1, :] = [sqrt(m.s.J), sqrt(m.s.θ), m.s.w]
    return coefs
end
...
function solve(m;damp=0.3, deg=2, tol=1e-7, verbose::Bool=true, coefs_init::Union{Matrix{Float64}, Nothing}=nothing)
    ...
    while error > tol
        ...
        out = step(m, J, θ, g.u, g.ξx, g.ξB, g.ξα, g.ξη)
        ...
        for node in 1:n_nodes
            ...
            J1[:, node] = (X1*coefs[:,1]).^2
            θ1[:, node] = (X1*coefs[:, 2]).^2
            w1[:, node] = X1*coefs[:, 3]

        end
        # Next-period quantities
        out = step(m, J1, θ1, u0, g.ξx1, g.ξB1, g.ξα1, g.ξη1)
        ...
        error = maximum(abs, 1.0.-J_new./J_old) +
                maximum(abs, 1.0.-θ_new./θ_old) +
...
function Simulation(m, coefs::Matrix; capT=50_000, burn_in=500)
    ...
    @inbounds for t in 2:(T)
        ...
        out = step(m, J[t], θ[t], u[t-1], ξx[t], ξB[t], ξα[t], ξη[t])
...
function main()
    p = Params(σB=1e-4, σα=1e-4, ση=1e-4  )
    m = Model(p, deg=2)
